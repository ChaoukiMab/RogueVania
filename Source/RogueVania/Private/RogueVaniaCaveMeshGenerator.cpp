    ...
    const FVector& GridOrigin)
{
    ...
    for (int32 i = 0; i < TunnelPoints.Num() - 1; i++)
    {
        ...
        {
            float Alpha = (NumSteps > 0) ? (float)Step / NumSteps : 0.0f;
            FVector CurrentPos = FMath::Lerp(StartPos, EndPos, Alpha);
    ...
    TArray<FVector2D>& UVs)
{
    ...
    for (const FVector& Vertex : QuadVertices)
    {
        Vertices.Add(Vertex * 100.0f); // Scale to world units
        Normals.Add(Normal);
        UVs.Add(FVector2D(0.0f, 0.0f)); // Simple UV mapping
    }

    // Create two triangles for the quad
    if (Direction.X < 0 || Direction.Y > 0 || Direction.Z > 0) // Flip winding for inward faces
    {
    ...
    const TArray<FVector2D>& UVs)
{
    ...
    // Get mesh description attributes
    FStaticMeshAttributes Attributes(*MeshDescription);
